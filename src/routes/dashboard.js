...
router.get('/ad-manager', authenticate, authorize(['AD_MANAGER', 'ADMIN']), async (req, res) => {
  try {
    const { timeframe = '30d' } = req.query;
    const days = { '7d': 7, '30d': 30, '90d': 90 }[timeframe] || 30;
    const fromDate = new Date();
...
router.get('/editor', authenticate, authorize(['EDITOR', 'AD_MANAGER', 'ADMIN']), async (req, res) => {
  try {
    const { timeframe = '30d' } = req.query;
    const days = { '7d': 7, '30d': 30, '90d': 90 }[timeframe] || 30;
    const fromDate = new Date();
...
// @access  Private (EDITOR, AD_MANAGER, ADMIN)
router.get('/content-performance', authenticate, authorize(['EDITOR', 'AD_MANAGER', 'ADMIN']), async (req, res) => {
  try {
 ...
 // Add these routes to your dashboard.js file
...
// @access  Private (AD_MANAGER, ADMIN)
router.get('/revenue', authenticate, authorize(['AD_MANAGER', 'ADMIN']), async (req, res) => {
  try {
    const { timeframe = '30d' } = req.query;
    const days = { '7d': 7, '30d': 30, '90d': 90 }[timeframe] || 30;
    const fromDate = new Date();
